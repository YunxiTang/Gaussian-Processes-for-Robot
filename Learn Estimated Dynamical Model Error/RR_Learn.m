...
x = x0;
mu = zeros(4,1);
sigma = 0.01 * diag([0.5,0.5,1,1]);
%% LEARN
% ode options
options = odeset('RelTol',1e-8, 'AbsTol',1e-8);
xargu = [0;0;0;0;x(1);x(2)];
...
for i=1:length(t_grid)
    ...
    query_x = [qrdd(1);qrdd(2);qrd(1);qrd(2);
               x(1);x(2)];
    ...
    t_s_grid = linspace(t_s(1),t_s(2),2);
    
    x_s = deval(sol,t_s_grid);
    
    x_next = x_s(:,2)+ mvnrnd(mu,sigma)';
    ...
    elseif mod(i,20) == 0
        ...
        Memory_X(:,indice) = data_point_x;  % insert the new coming data
        
        Yc1 = certain_delete(Yc1,indice,'col');
        Yc2 = certain_delete(Yc2,indice,'col');
        Yc1(:,indice) = data_point_y(1);
        Yc2(:,indice) = data_point_y(2);
    end
    disp('-------------NEXT STATE!!!------------------');
    disp(t_current);
    % training ...
    ...
    x = x_next;
    toc
    if mod(i,1000)==0 && t_current > 0
        e1 = ((Xr(1,:)-X(1,:)))/pi*180;
        e2 = ((Xr(2,:)-X(2,:)))/pi*180;
        RMSE_1 = sqrt((e1 * e1') / (t_current/dt));
        RMSE_2 = sqrt((e2 * e2') / (t_current/dt));
        rmse1 = [rmse1 RMSE_1];
        rmse2 = [rmse2 RMSE_2];
    end
end
...
figure(2);
subplot(2,1,1);
plot(t_grid,e1,'r','linewidth',2.0);
title('Error_{q1}');
grid on;
subplot(2,1,2);
